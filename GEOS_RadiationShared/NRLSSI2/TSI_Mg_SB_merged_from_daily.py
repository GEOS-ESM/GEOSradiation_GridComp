...
class TSI_Mg_SB_Merged_Daily:
  ...
  def _plot_all_final(self):
      ...
      plt.subplot(311); plt.plot(dy, TSI, '-'); plt.ylabel('TSI')
      plt.subplot(312); plt.plot(dy,  Mg,  '-'); plt.ylabel('Mg')
      plt.subplot(313); plt.plot(dy,  SB,  '-'); plt.ylabel('SB')
  ...
  def _shade_v03_block(self, dy):
      ...
      for ax in axes:
          ...
          ax.axvspan(bdy_years, x_end, facecolor='lightgoldenrodyellow', alpha=0.6, zorder=0)
          # vertical boundary line
          ax.axvline(bdy_years, color='k', linestyle='--', lw=1.1, zorder=3)
          # thin top ribbon + centered label above data
          trans = blended_transform_factory(ax.transData, ax.transAxes)
          ax.axvspan(bdy_years, x_end, ymin=0.965, ymax=0.985, facecolor='#ffd54f', alpha=0.9, zorder=4)
          ax.text((bdy_years + x_end)/2.0, 0.982, 'v03r00 mapped to legacy units',
                  transform=trans, ha='center', va='top', fontsize=8, zorder=5)
  
      # legend in bottom panel
      legend_patch = Patch(facecolor='lightgoldenrodyellow', edgecolor='none',
...
# print(t, z.gettime(t))
  ...
  z._plot_all_final()
  plt.savefig(os.sep.join(('gx','TSInMgSB_plot_all_final.png')), pad_inches=0.33,bbox_inches='tight',dpi=100)
  plt.show()
