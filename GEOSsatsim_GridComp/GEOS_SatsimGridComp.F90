...
!BOP
! !MODULE: GEOS_Satsim  -- Drive satellite simulators using grid mean cloud parameters

! !DESCRIPTION:
! 
!   {\tt GEOS\_SatsimGridCompMod} simulates various satellite platform instruments using
!   approximate forward operators provided by various groups. Implicit is the generation
!   of sub-gridscale cloud features from grid mean properties.
!
! !USES:
...
! !DESCRIPTION:  {\tt GEOS\_SatsimGridCompMod} uses the default Initialize
!                and Finalize services, but registers its own Run method.
...
! Local derived type aliases and variables

    type (MAPL_MetaComp), pointer :: STATE 
    type (ESMF_Config)            :: CF

    type (SatSim_State), pointer  :: self => null()  ! internal state
...
!  
!   Supercategories further subvided into high/over(O), {middle(M)},
!   or low/under(U) and thin (A) and thick (B)
!
!   In addition, 7 categories of subvisual (Sub, low tau) clouds with
!     with Sub_1 having P_top above 180 hPa, Sub_2 having P_top between
!     180. and 310., ... etc
!
...
! ------------------------
    ...
    call MAPL_TimerAdd(GC,name="-COSP" ,__RC__)
...
! -----------------------------
    ...
    if (found) then
       call ESMF_ConfigLoadFile(CF,'SatSim.rc',__RC__)
       call ESMF_ConfigGetDim(CF,nLines,nCols,LABEL='Masked_Exports::',RC=STATUS)
       if (status==ESMF_SUCCESS) then
          self%nmask_vars = nLines
          allocate(self%export_name(nLines),self%mask_name(nLines),__STAT__)
          allocate(self%newvar_name(nLines),self%newvar(nLines),__STAT__)
          call ESMF_ConfigFindLabel(CF,'Masked_Exports::',__RC__)
          do m=1,nLines
             call ESMF_ConfigNextLine(CF,__RC__)
             call ESMF_ConfigGetAttribute(CF,self%export_name(m),__RC__)
             call ESMF_ConfigGetAttribute(CF,self%mask_name(m),__RC__)
...
! Set up newly named EXPORT for a masked export, where requested
! --------------------------------------------------------------
    ...
    do m=1,self%nmask_vars
       if (self%newvar(m)) then
          call MAPL_Get(STATE,ExportSpec=ExportSpec,__RC__)
          vindex = MAPL_VarSpecGetIndex(ExportSpec,self%export_name(m),__RC__)
          call MAPL_VarSpecGet(ExportSpec(vindex),LONG_NAME=long_name, &
             UNITS=units,Dims=MAPL_dims,VLocation=VLocation,ungridded_dims=Ungridded_dims,__RC__)
          ...
          call MAPL_DoNotDeferExport(GC,exportName,__RC__)
       end if
       call MAPL_DoNotDeferExport(GC,self%export_name,__RC__)
    end do
...
! "stacked" fields (im*jm*7) deprecated
...
!-----------------------------------
    ...
    call SIM_DRIVER(IM,JM,LM,__RC__)
...
! These are the same, converted to upside-down and 2d 
      ...
      real, dimension(IM*JM,0:LM) :: PLECOSP, ZLECOSP 
...
!  The following definitions are for variables needed for COSP
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!  The following definitions are for variables needed for COSP and isccp/icarus
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    real                      :: K2
    integer :: use_mie_tables    ! use a precomputed lookup table? yes=1,no=0,
                                 !   2=use first column everywhere
    integer :: use_gas_abs       ! include gaseous absorption? yes=1,no=0
    integer :: do_ray            ! calculate/output Rayleigh refl=1, not=0
    integer :: Nprmts_max_hydro  ! Max num of parameters for hydrometeor size distributions
    integer :: Nprmts_max_aero   ! Max num of parameters for aerosol size distributions
    real    :: radar_freq
...
! suffixes indicate the cosp return structures that the variables are copied out of
    ...
    integer, dimension(IM*JM,LM) :: n_valid_subgridbox_values
...
! Which simulators are required?
    call MAPL_GetResource(MAPL,USE_SATSIM,      LABEL="USE_SATSIM:",      default=0, __RC__)
    call MAPL_GetResource(MAPL,USE_SATSIM_ISCCP,LABEL="USE_SATSIM_ISCCP:",default=0, __RC__)
    call MAPL_GetResource(MAPL,USE_SATSIM_MODIS,LABEL="USE_SATSIM_MODIS:",default=0, __RC__)
    call MAPL_GetResource(MAPL,USE_SATSIM_RADAR,LABEL="USE_SATSIM_RADAR:",default=0, __RC__)
    call MAPL_GetResource(MAPL,USE_SATSIM_LIDAR,LABEL="USE_SATSIM_LIDAR:",default=0, __RC__)
    call MAPL_GetResource(MAPL,USE_SATSIM_MISR, LABEL="USE_SATSIM_MISR:", default=0, __RC__)

! Set number of subcolumns ("Ncolumns")
! In Satsim parlance a "point" is a gridcolumn and a "column" is a subcolumn
    ...
    ! higher resolutions needs less subcolumns
    associate (default_Ncolumns => MIN(30,MAX(1,INT(4*5760*4/imsize))))
      call MAPL_GetResource(MAPL,Ncolumns,LABEL="SATSIM_NCOLUMNS:",default=default_Ncolumns, __RC__)
    end associate
...
! Possible partitioning of all points into groups. Used?
    call MAPL_GetResource(MAPL,Npoints_it,LABEL="SATSIM_POINTS_PER_ITERATION:",default=-999, __RC__)
...
!--------------------
    ...
    call MAPL_GetPointer(IMPORT, QV,     'QV',      __RC__)
    call MAPL_GetPointer(IMPORT, RDFL,   'RL',      __RC__)
    call MAPL_GetPointer(IMPORT, RDFI,   'RI',      __RC__)
    call MAPL_GetPointer(IMPORT, RDFR,   'RR',      __RC__)
    call MAPL_GetPointer(IMPORT, RDFS,   'RS',      __RC__)
    call MAPL_GetPointer(IMPORT, FCLD,   'FCLD',    __RC__)
    call MAPL_GetPointer(IMPORT, QLLS,   'QLLS',    __RC__)
    call MAPL_GetPointer(IMPORT, QILS,   'QILS',    __RC__)
    call MAPL_GetPointer(IMPORT, QLCN,   'QLCN',    __RC__)
    call MAPL_GetPointer(IMPORT, QICN,   'QICN',    __RC__)
    call MAPL_GetPointer(IMPORT, QRTOT,  'QRTOT',   __RC__)
    call MAPL_GetPointer(IMPORT, QSTOT,  'QSTOT',   __RC__)
    call MAPL_GetPointer(IMPORT, MCOSZ,  'MCOSZ',   __RC__)
...
! ISCCP/Icarus
    ...
    call MAPL_GetPointer(EXPORT, ISCCP_CU_OA,  'ISCCP_CU_OA',    __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CU_UA,  'ISCCP_CU_UA',    __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CU_OB,  'ISCCP_CU_OB',    __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CU_UB,  'ISCCP_CU_UB',    __RC__)

    call MAPL_GetPointer(EXPORT, ISCCP_STCU_OA, 'ISCCP_STCU_OA', __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_STCU_UA, 'ISCCP_STCU_UA', __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_STCU_OB, 'ISCCP_STCU_OB', __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_STCU_UB, 'ISCCP_STCU_UB', __RC__)

    call MAPL_GetPointer(EXPORT, ISCCP_ST_OA,   'ISCCP_ST_OA',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_ST_UA,   'ISCCP_ST_UA',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_ST_OB,   'ISCCP_ST_OB',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_ST_UB,   'ISCCP_ST_UB',   __RC__)

    call MAPL_GetPointer(EXPORT, ISCCP_ACU_OA,  'ISCCP_ACU_OA',  __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_ACU_UA,  'ISCCP_ACU_UA',  __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_ACU_OB,  'ISCCP_ACU_OB',  __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_ACU_UB,  'ISCCP_ACU_UB',  __RC__)

    call MAPL_GetPointer(EXPORT, ISCCP_AST_OA,  'ISCCP_AST_OA',  __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_AST_UA,  'ISCCP_AST_UA',  __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_AST_OB,  'ISCCP_AST_OB',  __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_AST_UB,  'ISCCP_AST_UB',  __RC__)

    call MAPL_GetPointer(EXPORT, ISCCP_NST_OA,  'ISCCP_NST_OA',  __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_NST_UA,  'ISCCP_NST_UA',  __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_NST_OB,  'ISCCP_NST_OB',  __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_NST_UB,  'ISCCP_NST_UB',  __RC__)

    call MAPL_GetPointer(EXPORT, ISCCP_CI_OA,   'ISCCP_CI_OA',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CI_MA,   'ISCCP_CI_MA',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CI_UA,   'ISCCP_CI_UA',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CI_OB,   'ISCCP_CI_OB',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CI_MB,   'ISCCP_CI_MB',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CI_UB,   'ISCCP_CI_UB',   __RC__)

    call MAPL_GetPointer(EXPORT, ISCCP_CIST_OA, 'ISCCP_CIST_OA', __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CIST_MA, 'ISCCP_CIST_MA', __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CIST_UA, 'ISCCP_CIST_UA', __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CIST_OB, 'ISCCP_CIST_OB', __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CIST_MB, 'ISCCP_CIST_MB', __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CIST_UB, 'ISCCP_CIST_UB', __RC__)

    call MAPL_GetPointer(EXPORT, ISCCP_CB_OA,   'ISCCP_CB_OA',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CB_MA,   'ISCCP_CB_MA',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CB_UA,   'ISCCP_CB_UA',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CB_OB,   'ISCCP_CB_OB',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CB_MB,   'ISCCP_CB_MB',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_CB_UB,   'ISCCP_CB_UB',   __RC__)

    call MAPL_GetPointer(EXPORT, ISCCP_SUBV1,   'ISCCP_SUBV1',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_SUBV2,   'ISCCP_SUBV2',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_SUBV3,   'ISCCP_SUBV3',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_SUBV4,   'ISCCP_SUBV4',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_SUBV5,   'ISCCP_SUBV5',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_SUBV6,   'ISCCP_SUBV6',   __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP_SUBV7,   'ISCCP_SUBV7',   __RC__)

    call MAPL_GetPointer(EXPORT, CLISCCP1,      'CLISCCP1',      __RC__)
    call MAPL_GetPointer(EXPORT, CLISCCP2,      'CLISCCP2',      __RC__)
    call MAPL_GetPointer(EXPORT, CLISCCP3,      'CLISCCP3',      __RC__)
    call MAPL_GetPointer(EXPORT, CLISCCP4,      'CLISCCP4',      __RC__)
    call MAPL_GetPointer(EXPORT, CLISCCP5,      'CLISCCP5',      __RC__)
    call MAPL_GetPointer(EXPORT, CLISCCP6,      'CLISCCP6',      __RC__)
    call MAPL_GetPointer(EXPORT, CLISCCP7,      'CLISCCP7',      __RC__)

    call MAPL_GetPointer(EXPORT, ISCCP1,        'ISCCP1',        __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP2,        'ISCCP2',        __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP3,        'ISCCP3',        __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP4,        'ISCCP4',        __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP5,        'ISCCP5',        __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP6,        'ISCCP6',        __RC__)
    call MAPL_GetPointer(EXPORT, ISCCP7,        'ISCCP7',        __RC__)

    call MAPL_GetPointer(EXPORT, TCLISCCP,      'TCLISCCP',      __RC__)
    call MAPL_GetPointer(EXPORT, CLTISCCP,      'CLTISCCP',      __RC__)
    call MAPL_GetPointer(EXPORT, PCTISCCP,      'PCTISCCP',      __RC__)
    call MAPL_GetPointer(EXPORT, CTPISCCP,      'CTPISCCP',      __RC__)
    call MAPL_GetPointer(EXPORT, ALBISCCP,      'ALBISCCP',      __RC__)
    call MAPL_GetPointer(EXPORT, TBISCCP,       'TBISCCP',       __RC__)
    call MAPL_GetPointer(EXPORT, SGFCLD,        'SGFCLD',        __RC__)

! LIDAR/CALIPSO/PARASOL

    call MAPL_GetPointer(EXPORT, RADARLTCC,   'RADARLTCC',   __RC__)
    call MAPL_GetPointer(EXPORT, CLCALIPSO2,  'CLCALIPSO2',  __RC__)
    call MAPL_GetPointer(EXPORT, LIDARPMOL,   'LIDARPMOL',   __RC__)
    call MAPL_GetPointer(EXPORT, LIDARPTOT,   'LIDARPTOT',   __RC__)
    call MAPL_GetPointer(EXPORT, LIDARTAUTOT, 'LIDARTAUTOT', __RC__)
    call MAPL_GetPointer(EXPORT, RADARZETOT,  'RADARZETOT',  __RC__)
    call MAPL_GetPointer(EXPORT, CLCALIPSO,   'CLCALIPSO',   __RC__)
    call MAPL_GetPointer(EXPORT, CLLCALIPSO,  'CLLCALIPSO',  __RC__)
    call MAPL_GetPointer(EXPORT, CLMCALIPSO,  'CLMCALIPSO',  __RC__)
    call MAPL_GetPointer(EXPORT, CLHCALIPSO,  'CLHCALIPSO',  __RC__)
    call MAPL_GetPointer(EXPORT, CLTCALIPSO,  'CLTCALIPSO',  __RC__)

    call MAPL_GetPointer(EXPORT, PARASOLREFL0, 'PARASOLREFL0', __RC__)
    call MAPL_GetPointer(EXPORT, PARASOLREFL1, 'PARASOLREFL1', __RC__)
    call MAPL_GetPointer(EXPORT, PARASOLREFL2, 'PARASOLREFL2', __RC__)
    call MAPL_GetPointer(EXPORT, PARASOLREFL3, 'PARASOLREFL3', __RC__)
    call MAPL_GetPointer(EXPORT, PARASOLREFL4, 'PARASOLREFL4', __RC__)
    call MAPL_GetPointer(EXPORT, PARASOLREFL5, 'PARASOLREFL5', __RC__)

    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_01, 'CFADLIDARSR532_01', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_02, 'CFADLIDARSR532_02', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_03, 'CFADLIDARSR532_03', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_04, 'CFADLIDARSR532_04', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_05, 'CFADLIDARSR532_05', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_06, 'CFADLIDARSR532_06', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_07, 'CFADLIDARSR532_07', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_08, 'CFADLIDARSR532_08', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_09, 'CFADLIDARSR532_09', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_10, 'CFADLIDARSR532_10', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_11, 'CFADLIDARSR532_11', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_12, 'CFADLIDARSR532_12', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_13, 'CFADLIDARSR532_13', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_14, 'CFADLIDARSR532_14', __RC__)
    call MAPL_GetPointer(EXPORT, CFADLIDARSR532_15, 'CFADLIDARSR532_15', __RC__)

! RADAR/Cloudsat

    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD01, 'CLOUDSATCFAD01', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD02, 'CLOUDSATCFAD02', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD03, 'CLOUDSATCFAD03', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD04, 'CLOUDSATCFAD04', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD05, 'CLOUDSATCFAD05', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD06, 'CLOUDSATCFAD06', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD07, 'CLOUDSATCFAD07', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD08, 'CLOUDSATCFAD08', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD09, 'CLOUDSATCFAD09', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD10, 'CLOUDSATCFAD10', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD11, 'CLOUDSATCFAD11', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD12, 'CLOUDSATCFAD12', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD13, 'CLOUDSATCFAD13', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD14, 'CLOUDSATCFAD14', __RC__)
    call MAPL_GetPointer(EXPORT, CLOUDSATCFAD15, 'CLOUDSATCFAD15', __RC__)

! MODIS

    call MAPL_GetPointer(EXPORT, MDSCLDFRCTTL,    'MDSCLDFRCTTL',    __RC__)
    call MAPL_GetPointer(EXPORT, MDSCLDFRCWTR,    'MDSCLDFRCWTR',    __RC__)
    call MAPL_GetPointer(EXPORT, MDSCLDFRCH2O,    'MDSCLDFRCH2O',    __RC__)
    call MAPL_GetPointer(EXPORT, MDSCLDFRCICE,    'MDSCLDFRCICE',    __RC__)
    call MAPL_GetPointer(EXPORT, MDSCLDFRCHI,     'MDSCLDFRCHI',     __RC__)
    call MAPL_GetPointer(EXPORT, MDSCLDFRCMID,    'MDSCLDFRCMID',    __RC__)
    call MAPL_GetPointer(EXPORT, MDSCLDFRCLO,     'MDSCLDFRCLO',     __RC__)
    call MAPL_GetPointer(EXPORT, MDSOPTHCKTTL,    'MDSOPTHCKTTL',    __RC__)
    call MAPL_GetPointer(EXPORT, MDSOPTHCKWTR,    'MDSOPTHCKWTR',    __RC__)
    call MAPL_GetPointer(EXPORT, MDSOPTHCKH2O,    'MDSOPTHCKH2O',    __RC__)
    call MAPL_GetPointer(EXPORT, MDSOPTHCKICE,    'MDSOPTHCKICE',    __RC__)
    call MAPL_GetPointer(EXPORT, MDSOPTHCKTTLLG,  'MDSOPTHCKTTLLG',  __RC__)
    call MAPL_GetPointer(EXPORT, MDSOPTHCKWTRLG,  'MDSOPTHCKWTRLG',  __RC__)
    call MAPL_GetPointer(EXPORT, MDSOPTHCKH2OLG,  'MDSOPTHCKH2OLG',  __RC__)
    call MAPL_GetPointer(EXPORT, MDSOPTHCKICELG,  'MDSOPTHCKICELG',  __RC__)
    call MAPL_GetPointer(EXPORT, MDSCLDSZWTR,     'MDSCLDSZWTR',     __RC__)
    call MAPL_GetPointer(EXPORT, MDSCLDSZH20,     'MDSCLDSZH20',     __RC__)
    call MAPL_GetPointer(EXPORT, MDSCLDSZICE,     'MDSCLDSZICE',     __RC__)
    call MAPL_GetPointer(EXPORT, MDSCLDTOPPS,     'MDSCLDTOPPS',     __RC__)
    call MAPL_GetPointer(EXPORT, MDSWTRPATH,      'MDSWTRPATH',      __RC__)
    call MAPL_GetPointer(EXPORT, MDSH2OPATH,      'MDSH2OPATH',      __RC__)
    call MAPL_GetPointer(EXPORT, MDSICEPATH,      'MDSICEPATH',      __RC__)

    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST11, 'MDSTAUPRSHIST11', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST12, 'MDSTAUPRSHIST12', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST13, 'MDSTAUPRSHIST13', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST14, 'MDSTAUPRSHIST14', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST15, 'MDSTAUPRSHIST15', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST16, 'MDSTAUPRSHIST16', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST17, 'MDSTAUPRSHIST17', __RC__)

    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST21, 'MDSTAUPRSHIST21', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST22, 'MDSTAUPRSHIST22', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST23, 'MDSTAUPRSHIST23', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST24, 'MDSTAUPRSHIST24', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST25, 'MDSTAUPRSHIST25', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST26, 'MDSTAUPRSHIST26', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST27, 'MDSTAUPRSHIST27', __RC__)

    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST31, 'MDSTAUPRSHIST31', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST32, 'MDSTAUPRSHIST32', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST33, 'MDSTAUPRSHIST33', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST34, 'MDSTAUPRSHIST34', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST35, 'MDSTAUPRSHIST35', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST36, 'MDSTAUPRSHIST36', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST37, 'MDSTAUPRSHIST37', __RC__)

    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST41, 'MDSTAUPRSHIST41', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST42, 'MDSTAUPRSHIST42', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST43, 'MDSTAUPRSHIST43', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST44, 'MDSTAUPRSHIST44', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST45, 'MDSTAUPRSHIST45', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST46, 'MDSTAUPRSHIST46', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST47, 'MDSTAUPRSHIST47', __RC__)

    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST51, 'MDSTAUPRSHIST51', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST52, 'MDSTAUPRSHIST52', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST53, 'MDSTAUPRSHIST53', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST54, 'MDSTAUPRSHIST54', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST55, 'MDSTAUPRSHIST55', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST56, 'MDSTAUPRSHIST56', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST57, 'MDSTAUPRSHIST57', __RC__)

    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST61, 'MDSTAUPRSHIST61', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST62, 'MDSTAUPRSHIST62', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST63, 'MDSTAUPRSHIST63', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST64, 'MDSTAUPRSHIST64', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST65, 'MDSTAUPRSHIST65', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST66, 'MDSTAUPRSHIST66', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST67, 'MDSTAUPRSHIST67', __RC__)

    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST71, 'MDSTAUPRSHIST71', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST72, 'MDSTAUPRSHIST72', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST73, 'MDSTAUPRSHIST73', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST74, 'MDSTAUPRSHIST74', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST75, 'MDSTAUPRSHIST75', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST76, 'MDSTAUPRSHIST76', __RC__)
    call MAPL_GetPointer(EXPORT, MDSTAUPRSHIST77, 'MDSTAUPRSHIST77', __RC__)

! MISR

    call MAPL_GetPointer(EXPORT, MISRMNCLDTP,    'MISRMNCLDTP',      __RC__)
    call MAPL_GetPointer(EXPORT, MISRCLDAREA,    'MISRCLDAREA',      __RC__)

    call MAPL_GetPointer(EXPORT, MISRLYRTP0,     'MISRLYRTP0',       __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP250,   'MISRLYRTP250',     __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP750,   'MISRLYRTP750',     __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP1250,  'MISRLYRTP1250',    __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP1750,  'MISRLYRTP1750',    __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP2250,  'MISRLYRTP2250',    __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP2750,  'MISRLYRTP2750',    __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP3500,  'MISRLYRTP3500',    __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP4500,  'MISRLYRTP4500',    __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP6000,  'MISRLYRTP6000',    __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP8000,  'MISRLYRTP8000',    __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP10000, 'MISRLYRTP10000',   __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP12000, 'MISRLYRTP12000',   __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP14000, 'MISRLYRTP14000',   __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP16000, 'MISRLYRTP16000',   __RC__)
    call MAPL_GetPointer(EXPORT, MISRLYRTP18000, 'MISRLYRTP18000',   __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ0,        'MISRFQ0',          __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ250,      'MISRFQ250',        __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ750,      'MISRFQ750',        __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ1250,     'MISRFQ1250',       __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ1750,     'MISRFQ1750',       __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ2250,     'MISRFQ2250',       __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ2750,     'MISRFQ2750',       __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ3500,     'MISRFQ3500',       __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ4500,     'MISRFQ4500',       __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ6000,     'MISRFQ6000',       __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ8000,     'MISRFQ8000',       __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ10000,    'MISRFQ10000',      __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ12000,    'MISRFQ12000',      __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ14000,    'MISRFQ14000',      __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ16000,    'MISRFQ16000',      __RC__)
    call MAPL_GetPointer(EXPORT, MISRFQ18000,    'MISRFQ18000',      __RC__)

!  The following assignments are for variables needed for COSP and isccp/icarus
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! pmn: note the above routines are for Chou-Suarez cloud optical properties
! ... perhaps we will update for RRTMG[P] optical properties in future?
...
!  The following assignments are for variables needed for all simulators
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    NPOINTS = IM*JM 
    ISCCP_OVERLAP = 3  !  overlap type: 1=max, 2=rand, 3=max/rand
                       !    used by SCOPS, COSP and icarus
! pmn: make consistent with RadiationShared overlap scheme?

!  The following assignments are for variables needed for COSP
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   Naero=1  ! Number of aerosol species (Not used)
   if (Npoints_it == -999) Npoints_it=NPOINTS  ! Num of gridpoints processed in one iteration (probably not used)
...
!   Remember these are integers so + is logical OR, not AND.
    ...
    ! isccp needed for modis
    if ( USE_SATSIM + USE_SATSIM_ISCCP + USE_SATSIM_MODIS > 0 ) then
      cfg%Lisccp_sim = .true.
    else 
...
!  The following assignments are for variables needed for COSP and lidar_simulator
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    lidar_ice_type=0  ! Ice particle shape in lidar calculations (0=ice-spheres ; 1=ice-non-spherical)

!  The following assignments are for variables needed for COSP and radar_simulator
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
! <<< ???
    ...
    where (gbx%mr_hydro < 0.) gbx%mr_hydro=0.
...
! needed for CFMIP
    ...
    if ( USE_SATSIM + USE_SATSIM_ISCCP > 0 ) then 
      ! cosp reverses the pressure levels of the isccp matrix for some reason
      ...
      where (fq_isccp3D < -10.) fq_isccp3D=MAPL_UNDEF
      where (fq_isccp3D >  10.) fq_isccp3D=MAPL_UNDEF
      where (ISCCP_totalcldarea < 0.) ISCCP_totalcldarea=MAPL_UNDEF
      where (ISCCP_totalcldarea > 1.) ISCCP_totalcldarea=MAPL_UNDEF
      where (ISCCP_meanptop <= 0. ) ISCCP_meanptop=MAPL_UNDEF
      where (ISCCP_meanalbedocld < 0.) ISCCP_meanalbedocld=MAPL_UNDEF
      where (ISCCP_meanalbedocld > 1.) ISCCP_meanalbedocld=MAPL_UNDEF
      where (ISCCP_meanallskybrighttemp < 0.) ISCCP_meanallskybrighttemp=MAPL_UNDEF
#endif 
    endif
    ...
    if ( USE_SATSIM_MODIS + USE_SATSIM > 0 ) then
      ...
      where (MODIS_Optical_Thickness_Total_Mean    <= 0.) MODIS_Optical_Thickness_Total_Mean         = MAPL_UNDEF
      where (modis_Optical_Thickness_Water_Mean    <= 0.) modis_Optical_Thickness_Water_Mean         = MAPL_UNDEF
      where (modis_Optical_Thickness_Ice_Mean      <= 0.) modis_Optical_Thickness_Ice_Mean           = MAPL_UNDEF
      where (modis_Optical_Thickness_Total_LogMean <= 0.) modis_Optical_Thickness_Total_LogMean      = MAPL_UNDEF
      where (modis_Optical_Thickness_Water_LogMean <= 0.) modis_Optical_Thickness_Water_LogMean      = MAPL_UNDEF
      where (modis_Optical_Thickness_Ice_LogMean   <= 0.) modis_Optical_Thickness_Ice_LogMean        = MAPL_UNDEF
      where (modis_Cloud_Particle_Size_Water_Mean  <= 0.) modis_Cloud_Particle_Size_Water_Mean       = MAPL_UNDEF
      where (modis_Cloud_Particle_Size_Ice_Mean    <= 0.) modis_Cloud_Particle_Size_Ice_Mean         = MAPL_UNDEF
      ...
      where (MODIS_Cloud_Top_Pressure_Total_Mean  <= 0.)       MODIS_Cloud_Top_Pressure_Total_Mean   = MAPL_UNDEF
    ...
    if ( USE_SATSIM_LIDAR + USE_SATSIM > 0 ) then
      ...
      where (lidar_lidarcld    > 1.      ) lidar_lidarcld    = MAPL_UNDEF 
      where (lidar_lidarcld    < 0.      ) lidar_lidarcld    = MAPL_UNDEF 
    ...
    if ( USE_SATSIM_MISR + USE_SATSIM > 0 ) then
      ...
      where (MISR_meanztop             <= 0.      ) MISR_meanztop             = MAPL_UNDEF
...
! 4 Cumulus (CU) subcategories
!-------------------------------------------------
    if (associated(ISCCP_CU_OA))   ISCCP_CU_OA    = fq_isccp3D(:,:,2,6)
    if (associated(ISCCP_CU_OB))   ISCCP_CU_OB    = fq_isccp3D(:,:,3,6)
    if (associated(ISCCP_CU_UA))   ISCCP_CU_UA    = fq_isccp3D(:,:,2,7)
    if (associated(ISCCP_CU_UB))   ISCCP_CU_UB    = fq_isccp3D(:,:,3,7)

! 4 Stratocumulus STCU subcategories
!-------------------------------------------------
    if (associated(ISCCP_STCU_OA)) ISCCP_STCU_OA  = fq_isccp3D(:,:,4,6)
    if (associated(ISCCP_STCU_OB)) ISCCP_STCU_OB  = fq_isccp3D(:,:,5,6)
    if (associated(ISCCP_STCU_UA)) ISCCP_STCU_UA  = fq_isccp3D(:,:,4,7)
    if (associated(ISCCP_STCU_UB)) ISCCP_STCU_UB  = fq_isccp3D(:,:,5,7)

! 4 Stratus ST subcategories
!-------------------------------------------------
    if (associated(ISCCP_ST_OA))   ISCCP_ST_OA    = fq_isccp3D(:,:,6,6)
    if (associated(ISCCP_ST_OB))   ISCCP_ST_OB    = fq_isccp3D(:,:,7,6)
    if (associated(ISCCP_ST_UA))   ISCCP_ST_UA    = fq_isccp3D(:,:,6,7)
    if (associated(ISCCP_ST_UB))   ISCCP_ST_UB    = fq_isccp3D(:,:,7,7)

! 4 Altocumulus ACU subcategories
!-------------------------------------------------
    if (associated(ISCCP_ACU_OA))  ISCCP_ACU_OA   = fq_isccp3D(:,:,2,4)
    if (associated(ISCCP_ACU_OB))  ISCCP_ACU_OB   = fq_isccp3D(:,:,3,4)
    if (associated(ISCCP_ACU_UA))  ISCCP_ACU_UA   = fq_isccp3D(:,:,2,5)
    if (associated(ISCCP_ACU_UB))  ISCCP_ACU_UB   = fq_isccp3D(:,:,3,5)

! 4 Altostratus AST subcategories
!-------------------------------------------------
    if (associated(ISCCP_AST_OA))  ISCCP_AST_OA   = fq_isccp3D(:,:,4,4)
    if (associated(ISCCP_AST_OB))  ISCCP_AST_OB   = fq_isccp3D(:,:,5,4)
    if (associated(ISCCP_AST_UA))  ISCCP_AST_UA   = fq_isccp3D(:,:,4,5)
    if (associated(ISCCP_AST_UB))  ISCCP_AST_UB   = fq_isccp3D(:,:,5,5)

! 4 Nimbostratus NST subcategories
!-------------------------------------------------
    if (associated(ISCCP_NST_OA))  ISCCP_NST_OA   = fq_isccp3D(:,:,6,4)
    if (associated(ISCCP_NST_OB))  ISCCP_NST_OB   = fq_isccp3D(:,:,7,4)
    if (associated(ISCCP_NST_UA))  ISCCP_NST_UA   = fq_isccp3D(:,:,6,5)
    if (associated(ISCCP_NST_UB))  ISCCP_NST_UB   = fq_isccp3D(:,:,7,5)

! 6 Cirrus CI subcategories
!-------------------------------------------------
    if (associated(ISCCP_CI_OA))   ISCCP_CI_OA    = fq_isccp3D(:,:,2,1)
    if (associated(ISCCP_CI_OB))   ISCCP_CI_OB    = fq_isccp3D(:,:,3,1)
    if (associated(ISCCP_CI_MA))   ISCCP_CI_MA    = fq_isccp3D(:,:,2,2)
    if (associated(ISCCP_CI_MB))   ISCCP_CI_MB    = fq_isccp3D(:,:,3,2)
    if (associated(ISCCP_CI_UA))   ISCCP_CI_UA    = fq_isccp3D(:,:,2,3)
    if (associated(ISCCP_CI_UB))   ISCCP_CI_UB    = fq_isccp3D(:,:,3,3)

! 6 Cirrostratus CIST subcategories
!-------------------------------------------------
    if (associated(ISCCP_CIST_OA)) ISCCP_CIST_OA  = fq_isccp3D(:,:,4,1)
    if (associated(ISCCP_CIST_OB)) ISCCP_CIST_OB  = fq_isccp3D(:,:,5,1)
    if (associated(ISCCP_CIST_MA)) ISCCP_CIST_MA  = fq_isccp3D(:,:,4,2)
    if (associated(ISCCP_CIST_MB)) ISCCP_CIST_MB  = fq_isccp3D(:,:,5,2)
    if (associated(ISCCP_CIST_UA)) ISCCP_CIST_UA  = fq_isccp3D(:,:,4,3)
    if (associated(ISCCP_CIST_UB)) ISCCP_CIST_UB  = fq_isccp3D(:,:,5,3)

! 6 Cumulonimbus CB subcategories
!-------------------------------------------------
    if (associated(ISCCP_CB_OA))   ISCCP_CB_OA    = fq_isccp3D(:,:,6,1)
    if (associated(ISCCP_CB_OB))   ISCCP_CB_OB    = fq_isccp3D(:,:,7,1)
    if (associated(ISCCP_CB_MA))   ISCCP_CB_MA    = fq_isccp3D(:,:,6,2)
    if (associated(ISCCP_CB_MB))   ISCCP_CB_MB    = fq_isccp3D(:,:,7,2)
    if (associated(ISCCP_CB_UA))   ISCCP_CB_UA    = fq_isccp3D(:,:,6,3)
    if (associated(ISCCP_CB_UB))   ISCCP_CB_UB    = fq_isccp3D(:,:,7,3)

! 7 Subvisible/Subdetection (SUBV) subcategories  
!-------------------------------------------------
    if (associated(ISCCP_SUBV1))   ISCCP_SUBV1    = fq_isccp3D(:,:,1,1)
    if (associated(ISCCP_SUBV2))   ISCCP_SUBV2    = fq_isccp3D(:,:,1,2)
    if (associated(ISCCP_SUBV3))   ISCCP_SUBV3    = fq_isccp3D(:,:,1,3)
    if (associated(ISCCP_SUBV4))   ISCCP_SUBV4    = fq_isccp3D(:,:,1,4)
    if (associated(ISCCP_SUBV5))   ISCCP_SUBV5    = fq_isccp3D(:,:,1,5)
    if (associated(ISCCP_SUBV6))   ISCCP_SUBV6    = fq_isccp3D(:,:,1,6)
    if (associated(ISCCP_SUBV7))   ISCCP_SUBV7    = fq_isccp3D(:,:,1,7)

! 7 pressure levels of all thicknesses  
!-------------------------------------------------
    if (associated(CLISCCP1))      CLISCCP1       = fq_isccp3D(:,:,:,1)
    if (associated(CLISCCP2))      CLISCCP2       = fq_isccp3D(:,:,:,2)
    if (associated(CLISCCP3))      CLISCCP3       = fq_isccp3D(:,:,:,3)
    if (associated(CLISCCP4))      CLISCCP4       = fq_isccp3D(:,:,:,4)
    if (associated(CLISCCP5))      CLISCCP5       = fq_isccp3D(:,:,:,5)
    if (associated(CLISCCP6))      CLISCCP6       = fq_isccp3D(:,:,:,6)
    if (associated(CLISCCP7))      CLISCCP7       = fq_isccp3D(:,:,:,7)

    if (associated(ISCCP1))        ISCCP1         = fq_isccp3D(:,:,:,1)
    if (associated(ISCCP2))        ISCCP2         = fq_isccp3D(:,:,:,2)
    if (associated(ISCCP3))        ISCCP3         = fq_isccp3D(:,:,:,3)
    if (associated(ISCCP4))        ISCCP4         = fq_isccp3D(:,:,:,4)
    if (associated(ISCCP5))        ISCCP5         = fq_isccp3D(:,:,:,5)
    if (associated(ISCCP6))        ISCCP6         = fq_isccp3D(:,:,:,6)
    if (associated(ISCCP7))        ISCCP7         = fq_isccp3D(:,:,:,7)

    if (associated(CLTISCCP)) CLTISCCP  = reshape( ISCCP_totalcldarea,         (/ IM, JM /)) 
    if (associated(TCLISCCP)) TCLISCCP  = reshape( ISCCP_totalcldarea,         (/ IM, JM /)) 
    if (associated(PCTISCCP)) PCTISCCP  = reshape( ISCCP_meanptop,             (/ IM, JM /))
    if (associated(CTPISCCP)) CTPISCCP  = reshape( ISCCP_meanptop,             (/ IM, JM /))
    if (associated(ALBISCCP)) ALBISCCP  = reshape( ISCCP_meanalbedocld,        (/ IM, JM /))
    if (associated(TBISCCP))  TBISCCP   = reshape( ISCCP_meanallskybrighttemp, (/ IM, JM /))

! CALIPSO & PARASOL lidar exports
!-------------------------------------------------

    if (associated(CLCALIPSO))  CLCALIPSO  = reshape( lidar_lidarcld(:,LM:1:-1), (/IM,JM,LM/))
    if (associated(CLLCALIPSO)) CLLCALIPSO = reshape( lidar_cldlayer(:,1), (/IM,JM/)) ! low
    if (associated(CLMCALIPSO)) CLMCALIPSO = reshape( lidar_cldlayer(:,2), (/IM,JM/)) ! middle
    if (associated(CLHCALIPSO)) CLHCALIPSO = reshape( lidar_cldlayer(:,3), (/IM,JM/)) ! high
    if (associated(CLTCALIPSO)) CLTCALIPSO = reshape( lidar_cldlayer(:,4), (/IM,JM/)) ! total

    if (associated(PARASOLREFL0)) PARASOLREFL0 = reshape( lidar_parasolrefl, (/IM,JM,PARASOL_NREFL/))
    if (associated(PARASOLREFL1)) PARASOLREFL1 = reshape( lidar_parasolrefl(:,1), (/IM,JM/)) 
    if (associated(PARASOLREFL2)) PARASOLREFL2 = reshape( lidar_parasolrefl(:,2), (/IM,JM/)) 
    if (associated(PARASOLREFL3)) PARASOLREFL3 = reshape( lidar_parasolrefl(:,3), (/IM,JM/)) 
    if (associated(PARASOLREFL4)) PARASOLREFL4 = reshape( lidar_parasolrefl(:,4), (/IM,JM/)) 
    if (associated(PARASOLREFL5)) PARASOLREFL5 = reshape( lidar_parasolrefl(:,5), (/IM,JM/)) 

    if (associated(CFADLIDARSR532_01)) CFADLIDARSR532_01 = reshape( lidar_cfad_sr(:, 1,LM:1:-1), (/IM,JM,LM/))
    if (associated(CFADLIDARSR532_02)) CFADLIDARSR532_02 = reshape( lidar_cfad_sr(:, 2,LM:1:-1), (/IM,JM,LM/))
    if (associated(CFADLIDARSR532_03)) CFADLIDARSR532_03 = reshape( lidar_cfad_sr(:, 3,LM:1:-1), (/IM,JM,LM/))
    if (associated(CFADLIDARSR532_04)) CFADLIDARSR532_04 = reshape( lidar_cfad_sr(:, 4,LM:1:-1), (/IM,JM,LM/))
    if (associated(CFADLIDARSR532_05)) CFADLIDARSR532_05 = reshape( lidar_cfad_sr(:, 5,LM:1:-1), (/IM,JM,LM/))
    if (associated(CFADLIDARSR532_06)) CFADLIDARSR532_06 = reshape( lidar_cfad_sr(:, 6,LM:1:-1), (/IM,JM,LM/))
    if (associated(CFADLIDARSR532_07)) CFADLIDARSR532_07 = reshape( lidar_cfad_sr(:, 7,LM:1:-1), (/IM,JM,LM/)) 
    if (associated(CFADLIDARSR532_08)) CFADLIDARSR532_08 = reshape( lidar_cfad_sr(:, 8,LM:1:-1), (/IM,JM,LM/))
    if (associated(CFADLIDARSR532_09)) CFADLIDARSR532_09 = reshape( lidar_cfad_sr(:, 9,LM:1:-1), (/IM,JM,LM/)) 
    if (associated(CFADLIDARSR532_10)) CFADLIDARSR532_10 = reshape( lidar_cfad_sr(:,10,LM:1:-1), (/IM,JM,LM/)) 
    if (associated(CFADLIDARSR532_11)) CFADLIDARSR532_11 = reshape( lidar_cfad_sr(:,11,LM:1:-1), (/IM,JM,LM/)) 
    if (associated(CFADLIDARSR532_12)) CFADLIDARSR532_12 = reshape( lidar_cfad_sr(:,12,LM:1:-1), (/IM,JM,LM/)) 
    if (associated(CFADLIDARSR532_13)) CFADLIDARSR532_13 = reshape( lidar_cfad_sr(:,13,LM:1:-1), (/IM,JM,LM/)) 
    if (associated(CFADLIDARSR532_14)) CFADLIDARSR532_14 = reshape( lidar_cfad_sr(:,14,LM:1:-1), (/IM,JM,LM/)) 
    if (associated(CFADLIDARSR532_15)) CFADLIDARSR532_15 = reshape( lidar_cfad_sr(:,15,LM:1:-1), (/IM,JM,LM/)) 

    if (associated(LIDARPMOL))   LIDARPMOL   = reshape( lidar_pmol(:,LM:1:-1),                             (/IM,JM,LM/))
    if (associated(LIDARPTOT))   LIDARPTOT   = reshape( sum(lidar_beta_tot(:,:,LM:1:-1), 2)/(1.*NCOLumns), (/IM,JM,LM/))
    if (associated(LIDARTAUTOT)) LIDARTAUTOT = reshape( sum(lidar_tau_tot (:,:,LM:1:-1), 2)/(1.*NCOLumns), (/IM,JM,LM/))

! CLOUDSAT/radar exports
!-------------------------------------------------

    ! old code did not mask out invalid values and averaged in dBZ not reflectivity
    ! if (associated(RADARZETOT)) then
    !   RADARZETOT = reshape( sum(radar_ze_tot(:,:,LM:1:-1),2)/(1.*NCOLumns), (/IM,JM,LM/))
    ! endif

    ! New code calculates grid-box dBZ, but doing the underlying average in
    ! reflectivity space. It also correctly handles invalid sub-gridbox values.
    if (associated(RADARZETOT)) then 
      radar_ze_tot_mask = (radar_ze_tot /= MAPL_UNDEF)

      ! radar_ze_tot(IM*JM,NCOLUMNS,LM) so dim=2 is the sub-gridbox dimension.
      n_valid_subgridbox_values = count(radar_ze_tot_mask,dim=2)

      ! Find max valid sub-gridbox value and replicate to each subgrid point (_tmp).
      ! This will be -HUGE(real) for n_valid_subgridbox_values == 0 gridboxes.
      radar_ze_tot_max = maxval(radar_ze_tot,dim=2,mask=radar_ze_tot_mask)
      radar_ze_tot_tmp = spread(radar_ze_tot_max,dim=2,ncopies=NCOLUMNS)

      ! Convert valid values to relative reflectivity.
      ! Bypasses n_valid_subgridbox_values == 0 gridboxes, since all their
      !   sub-grid values are masked out. The subgrid _tmp values of the 
      !   bypassed gridboxes values retain their -HUGE values.
      where (radar_ze_tot_mask) &
        radar_ze_tot_tmp = 10.0**((radar_ze_tot-radar_ze_tot_tmp)/10.0)

      ! Find mean relative reflectivity for gridboxes with any valid sub-grid
      !   values, and convert back to dBZ as radar_ze_tot_mean(IM*JM,LM).
      ! Note: the sum() is non-elemental, so is evaluated before the where mask
      !   on n_valid_subgridbox_values > 0 is applied. But since the sum itself
      !   has mask=radar_ze_tot_mask, it only sums valid sub-gridbox locations.
      !   In particular, it excludes all the -HUGE values in radar_ze_tot_tmp
      !   because these are only present in gridboxes with all sub-grid values
      !   masked out.

      where (n_valid_subgridbox_values > 0)
        radar_ze_tot_mean = &
          sum(radar_ze_tot_tmp,dim=2,mask=radar_ze_tot_mask) &
          / real(n_valid_subgridbox_values)
        radar_ze_tot_mean = radar_ze_tot_max + 10.0*log10(radar_ze_tot_mean)
      elsewhere
        radar_ze_tot_mean = MAPL_UNDEF  ! gridbox has no valid subgrid values
      end where

      ! invert & reshape for output
      RADARZETOT = reshape(radar_ze_tot_mean(:,LM:1:-1),(/IM,JM,LM/))
    end if 

    if (associated(RADARLTCC))  RADARLTCC  = reshape(radar_lidar_tcc,(/IM,JM/))
    if (associated(CLCALIPSO2)) CLCALIPSO2 = reshape(radar_lidar_only_freq_cloud(:,LM:1:-1),(/IM,JM,LM/))
    if (associated(CLOUDSATCFAD01)) CLOUDSATCFAD01 = reshape(radar_cfad_ze(:, 1,LM:1:-1),(/IM,JM,LM/))
    if (associated(CLOUDSATCFAD02)) CLOUDSATCFAD02 = reshape(radar_cfad_ze(:, 2,LM:1:-1),(/IM,JM,LM/))
    if (associated(CLOUDSATCFAD03)) CLOUDSATCFAD03 = reshape(radar_cfad_ze(:, 3,LM:1:-1),(/IM,JM,LM/))
    if (associated(CLOUDSATCFAD04)) CLOUDSATCFAD04 = reshape(radar_cfad_ze(:, 4,LM:1:-1),(/IM,JM,LM/))
    if (associated(CLOUDSATCFAD05)) CLOUDSATCFAD05 = reshape(radar_cfad_ze(:, 5,LM:1:-1),(/IM,JM,LM/))
    if (associated(CLOUDSATCFAD06)) CLOUDSATCFAD06 = reshape(radar_cfad_ze(:, 6,LM:1:-1),(/IM,JM,LM/))
    if (associated(CLOUDSATCFAD07)) CLOUDSATCFAD07 = reshape(radar_cfad_ze(:, 7,LM:1:-1),(/IM,JM,LM/)) 
    if (associated(CLOUDSATCFAD08)) CLOUDSATCFAD08 = reshape(radar_cfad_ze(:, 8,LM:1:-1),(/IM,JM,LM/))
    if (associated(CLOUDSATCFAD09)) CLOUDSATCFAD09 = reshape(radar_cfad_ze(:, 9,LM:1:-1),(/IM,JM,LM/)) 
    if (associated(CLOUDSATCFAD10)) CLOUDSATCFAD10 = reshape(radar_cfad_ze(:,10,LM:1:-1),(/IM,JM,LM/)) 
    if (associated(CLOUDSATCFAD11)) CLOUDSATCFAD11 = reshape(radar_cfad_ze(:,11,LM:1:-1),(/IM,JM,LM/)) 
    if (associated(CLOUDSATCFAD12)) CLOUDSATCFAD12 = reshape(radar_cfad_ze(:,12,LM:1:-1),(/IM,JM,LM/)) 
    if (associated(CLOUDSATCFAD13)) CLOUDSATCFAD13 = reshape(radar_cfad_ze(:,13,LM:1:-1),(/IM,JM,LM/)) 
    if (associated(CLOUDSATCFAD14)) CLOUDSATCFAD14 = reshape(radar_cfad_ze(:,14,LM:1:-1),(/IM,JM,LM/)) 
    if (associated(CLOUDSATCFAD15)) CLOUDSATCFAD15 = reshape(radar_cfad_ze(:,15,LM:1:-1),(/IM,JM,LM/)) 

! MODIS exports -- currently available only from COSP

    if (associated(MDSCLDFRCTTL))    MDSCLDFRCTTL    = reshape(MODIS_Cloud_Fraction_Total_Mean,      (/IM,JM/)) 
    if (associated(MDSCLDFRCWTR))    MDSCLDFRCWTR    = reshape(MODIS_Cloud_Fraction_Water_Mean,      (/IM,JM/)) 
    if (associated(MDSCLDFRCH2O))    MDSCLDFRCH2O    = reshape(MODIS_Cloud_Fraction_Water_Mean,      (/IM,JM/)) 
    if (associated(MDSCLDFRCICE))    MDSCLDFRCICE    = reshape(MODIS_Cloud_Fraction_Ice_Mean,        (/IM,JM/)) 
    if (associated(MDSCLDFRCHI))     MDSCLDFRCHI     = reshape(MODIS_Cloud_Fraction_High_Mean,       (/IM,JM/)) 
    if (associated(MDSCLDFRCMID))    MDSCLDFRCMID    = reshape(MODIS_Cloud_Fraction_Mid_Mean,        (/IM,JM/)) 
    if (associated(MDSCLDFRCLO))     MDSCLDFRCLO     = reshape(MODIS_Cloud_Fraction_Low_Mean,        (/IM,JM/)) 
    if (associated(MDSOPTHCKTTL))    MDSOPTHCKTTL    = reshape(MODIS_Optical_Thickness_Total_Mean,   (/IM,JM/)) 
    if (associated(MDSOPTHCKWTR))    MDSOPTHCKWTR    = reshape(MODIS_Optical_Thickness_Water_Mean,   (/IM,JM/)) 
    if (associated(MDSOPTHCKH2O))    MDSOPTHCKH2O    = reshape(MODIS_Optical_Thickness_Water_Mean,   (/IM,JM/)) 
    if (associated(MDSOPTHCKICE))    MDSOPTHCKICE    = reshape(MODIS_Optical_Thickness_Ice_Mean,     (/IM,JM/)) 
    if (associated(MDSOPTHCKTTLLG))  MDSOPTHCKTTLLG  = reshape(MODIS_Optical_Thickness_Total_LogMean,(/IM,JM/)) 
    if (associated(MDSOPTHCKWTRLG))  MDSOPTHCKWTRLG  = reshape(MODIS_Optical_Thickness_Water_LogMean,(/IM,JM/)) 
    if (associated(MDSOPTHCKH2OLG))  MDSOPTHCKH2OLG  = reshape(MODIS_Optical_Thickness_Water_LogMean,(/IM,JM/)) 
    if (associated(MDSOPTHCKICELG))  MDSOPTHCKICELG  = reshape(MODIS_Optical_Thickness_Ice_LogMean,  (/IM,JM/)) 
    if (associated(MDSCLDSZWTR))     MDSCLDSZWTR     = reshape(MODIS_Cloud_Particle_Size_Water_Mean, (/IM,JM/)) 
    if (associated(MDSCLDSZH20))     MDSCLDSZH20     = reshape(MODIS_Cloud_Particle_Size_Water_Mean, (/IM,JM/)) 
    if (associated(MDSCLDSZICE))     MDSCLDSZICE     = reshape(MODIS_Cloud_Particle_Size_Ice_Mean,   (/IM,JM/)) 
    if (associated(MDSCLDTOPPS))     MDSCLDTOPPS     = reshape(MODIS_Cloud_Top_Pressure_Total_Mean,  (/IM,JM/)) 
    if (associated(MDSWTRPATH))      MDSWTRPATH      = reshape(MODIS_Liquid_Water_Path_Mean,         (/IM,JM/)) 
    if (associated(MDSH2OPATH))      MDSH2OPATH      = reshape(MODIS_Liquid_Water_Path_Mean,         (/IM,JM/)) 
    if (associated(MDSICEPATH))      MDSICEPATH      = reshape(MODIS_Ice_Water_Path_Mean,            (/IM,JM/)) 
    if (associated(MDSTAUPRSHIST11)) MDSTAUPRSHIST11 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,1,1),(/IM,JM/))
    if (associated(MDSTAUPRSHIST12)) MDSTAUPRSHIST12 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,1,2),(/IM,JM/))
    if (associated(MDSTAUPRSHIST13)) MDSTAUPRSHIST13 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,1,3),(/IM,JM/))
    if (associated(MDSTAUPRSHIST14)) MDSTAUPRSHIST14 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,1,4),(/IM,JM/))
    if (associated(MDSTAUPRSHIST15)) MDSTAUPRSHIST15 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,1,5),(/IM,JM/))
    if (associated(MDSTAUPRSHIST16)) MDSTAUPRSHIST16 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,1,6),(/IM,JM/))
    if (associated(MDSTAUPRSHIST17)) MDSTAUPRSHIST17 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,1,7),(/IM,JM/))
    if (associated(MDSTAUPRSHIST21)) MDSTAUPRSHIST21 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,2,1),(/IM,JM/))
    if (associated(MDSTAUPRSHIST22)) MDSTAUPRSHIST22 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,2,2),(/IM,JM/))
    if (associated(MDSTAUPRSHIST23)) MDSTAUPRSHIST23 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,2,3),(/IM,JM/))
    if (associated(MDSTAUPRSHIST24)) MDSTAUPRSHIST24 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,2,4),(/IM,JM/))
    if (associated(MDSTAUPRSHIST25)) MDSTAUPRSHIST25 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,2,5),(/IM,JM/))
    if (associated(MDSTAUPRSHIST26)) MDSTAUPRSHIST26 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,2,6),(/IM,JM/))
    if (associated(MDSTAUPRSHIST27)) MDSTAUPRSHIST27 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,2,7),(/IM,JM/))
    if (associated(MDSTAUPRSHIST31)) MDSTAUPRSHIST31 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,3,1),(/IM,JM/))
    if (associated(MDSTAUPRSHIST32)) MDSTAUPRSHIST32 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,3,2),(/IM,JM/))
    if (associated(MDSTAUPRSHIST33)) MDSTAUPRSHIST33 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,3,3),(/IM,JM/))
    if (associated(MDSTAUPRSHIST34)) MDSTAUPRSHIST34 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,3,4),(/IM,JM/))
    if (associated(MDSTAUPRSHIST35)) MDSTAUPRSHIST35 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,3,5),(/IM,JM/))
    if (associated(MDSTAUPRSHIST36)) MDSTAUPRSHIST36 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,3,6),(/IM,JM/))
    if (associated(MDSTAUPRSHIST37)) MDSTAUPRSHIST37 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,3,7),(/IM,JM/))
    if (associated(MDSTAUPRSHIST41)) MDSTAUPRSHIST41 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,4,1),(/IM,JM/))
    if (associated(MDSTAUPRSHIST42)) MDSTAUPRSHIST42 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,4,2),(/IM,JM/))
    if (associated(MDSTAUPRSHIST43)) MDSTAUPRSHIST43 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,4,3),(/IM,JM/))
    if (associated(MDSTAUPRSHIST44)) MDSTAUPRSHIST44 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,4,4),(/IM,JM/))
    if (associated(MDSTAUPRSHIST45)) MDSTAUPRSHIST45 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,4,5),(/IM,JM/))
    if (associated(MDSTAUPRSHIST46)) MDSTAUPRSHIST46 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,4,6),(/IM,JM/))
    if (associated(MDSTAUPRSHIST47)) MDSTAUPRSHIST47 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,4,7),(/IM,JM/))
    if (associated(MDSTAUPRSHIST51)) MDSTAUPRSHIST51 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,5,1),(/IM,JM/))
    if (associated(MDSTAUPRSHIST52)) MDSTAUPRSHIST52 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,5,2),(/IM,JM/))
    if (associated(MDSTAUPRSHIST53)) MDSTAUPRSHIST53 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,5,3),(/IM,JM/))
    if (associated(MDSTAUPRSHIST54)) MDSTAUPRSHIST54 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,5,4),(/IM,JM/))
    if (associated(MDSTAUPRSHIST55)) MDSTAUPRSHIST55 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,5,5),(/IM,JM/))
    if (associated(MDSTAUPRSHIST56)) MDSTAUPRSHIST56 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,5,6),(/IM,JM/))
    if (associated(MDSTAUPRSHIST57)) MDSTAUPRSHIST57 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,5,7),(/IM,JM/))
    if (associated(MDSTAUPRSHIST61)) MDSTAUPRSHIST61 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,6,1),(/IM,JM/))
    if (associated(MDSTAUPRSHIST62)) MDSTAUPRSHIST62 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,6,2),(/IM,JM/))
    if (associated(MDSTAUPRSHIST63)) MDSTAUPRSHIST63 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,6,3),(/IM,JM/))
    if (associated(MDSTAUPRSHIST64)) MDSTAUPRSHIST64 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,6,4),(/IM,JM/))
    if (associated(MDSTAUPRSHIST65)) MDSTAUPRSHIST65 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,6,5),(/IM,JM/))
    if (associated(MDSTAUPRSHIST66)) MDSTAUPRSHIST66 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,6,6),(/IM,JM/))
    if (associated(MDSTAUPRSHIST67)) MDSTAUPRSHIST67 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,6,7),(/IM,JM/))
    if (associated(MDSTAUPRSHIST71)) MDSTAUPRSHIST71 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,7,1),(/IM,JM/))
    if (associated(MDSTAUPRSHIST72)) MDSTAUPRSHIST72 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,7,2),(/IM,JM/))
    if (associated(MDSTAUPRSHIST73)) MDSTAUPRSHIST73 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,7,3),(/IM,JM/))
    if (associated(MDSTAUPRSHIST74)) MDSTAUPRSHIST74 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,7,4),(/IM,JM/))
    if (associated(MDSTAUPRSHIST75)) MDSTAUPRSHIST75 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,7,5),(/IM,JM/))
    if (associated(MDSTAUPRSHIST76)) MDSTAUPRSHIST76 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,7,6),(/IM,JM/))
    if (associated(MDSTAUPRSHIST77)) MDSTAUPRSHIST77 = reshape(MODIS_Optical_Thickness_vs_Cloud_Top_Pressure(:,7,7),(/IM,JM/))

! MISR exports

    if (associated(MISRMNCLDTP))    MISRMNCLDTP    = reshape(MISR_meanztop,(/IM,JM/)) 
    if (associated(MISRCLDAREA))    MISRCLDAREA    = reshape(MISR_cldarea, (/IM,JM/)) 
    if (associated(MISRLYRTP0))     MISRLYRTP0     = reshape(MISR_dist_model_layertops(:, 1),(/IM,JM/))
    if (associated(MISRLYRTP250))   MISRLYRTP250   = reshape(MISR_dist_model_layertops(:, 2),(/IM,JM/))
    if (associated(MISRLYRTP750))   MISRLYRTP750   = reshape(MISR_dist_model_layertops(:, 3),(/IM,JM/))
    if (associated(MISRLYRTP1250))  MISRLYRTP1250  = reshape(MISR_dist_model_layertops(:, 4),(/IM,JM/))
    if (associated(MISRLYRTP1750))  MISRLYRTP1750  = reshape(MISR_dist_model_layertops(:, 5),(/IM,JM/))
    if (associated(MISRLYRTP2250))  MISRLYRTP2250  = reshape(MISR_dist_model_layertops(:, 6),(/IM,JM/))
    if (associated(MISRLYRTP2750))  MISRLYRTP2750  = reshape(MISR_dist_model_layertops(:, 7),(/IM,JM/))
    if (associated(MISRLYRTP3500))  MISRLYRTP3500  = reshape(MISR_dist_model_layertops(:, 8),(/IM,JM/))
    if (associated(MISRLYRTP4500))  MISRLYRTP4500  = reshape(MISR_dist_model_layertops(:, 9),(/IM,JM/))
    if (associated(MISRLYRTP6000))  MISRLYRTP6000  = reshape(MISR_dist_model_layertops(:,10),(/IM,JM/))
    if (associated(MISRLYRTP8000))  MISRLYRTP8000  = reshape(MISR_dist_model_layertops(:,11),(/IM,JM/))
    if (associated(MISRLYRTP10000)) MISRLYRTP10000 = reshape(MISR_dist_model_layertops(:,12),(/IM,JM/))
    if (associated(MISRLYRTP12000)) MISRLYRTP12000 = reshape(MISR_dist_model_layertops(:,13),(/IM,JM/))
    if (associated(MISRLYRTP14000)) MISRLYRTP14000 = reshape(MISR_dist_model_layertops(:,14),(/IM,JM/))
    if (associated(MISRLYRTP16000)) MISRLYRTP16000 = reshape(MISR_dist_model_layertops(:,15),(/IM,JM/))
    if (associated(MISRLYRTP18000)) MISRLYRTP18000 = reshape(MISR_dist_model_layertops(:,16),(/IM,JM/))
    if (associated(MISRFQ0))        MISRFQ0        = reshape(fq_MISR(:,:, 1),(/IM,JM,7/))
    if (associated(MISRFQ250))      MISRFQ250      = reshape(fq_MISR(:,:, 2),(/IM,JM,7/))
    if (associated(MISRFQ750))      MISRFQ750      = reshape(fq_MISR(:,:, 3),(/IM,JM,7/))
    if (associated(MISRFQ1250))     MISRFQ1250     = reshape(fq_MISR(:,:, 4),(/IM,JM,7/))
    if (associated(MISRFQ1750))     MISRFQ1750     = reshape(fq_MISR(:,:, 5),(/IM,JM,7/))
    if (associated(MISRFQ2250))     MISRFQ2250     = reshape(fq_MISR(:,:, 6),(/IM,JM,7/))
    if (associated(MISRFQ2750))     MISRFQ2750     = reshape(fq_MISR(:,:, 7),(/IM,JM,7/))
    if (associated(MISRFQ3500))     MISRFQ3500     = reshape(fq_MISR(:,:, 8),(/IM,JM,7/))
    if (associated(MISRFQ4500))     MISRFQ4500     = reshape(fq_MISR(:,:, 9),(/IM,JM,7/))
    if (associated(MISRFQ6000))     MISRFQ6000     = reshape(fq_MISR(:,:,10),(/IM,JM,7/))
    if (associated(MISRFQ8000))     MISRFQ8000     = reshape(fq_MISR(:,:,11),(/IM,JM,7/))
    if (associated(MISRFQ10000))    MISRFQ10000    = reshape(fq_MISR(:,:,12),(/IM,JM,7/))
    if (associated(MISRFQ12000))    MISRFQ12000    = reshape(fq_MISR(:,:,13),(/IM,JM,7/))
    if (associated(MISRFQ14000))    MISRFQ14000    = reshape(fq_MISR(:,:,14),(/IM,JM,7/))
    if (associated(MISRFQ16000))    MISRFQ16000    = reshape(fq_MISR(:,:,15),(/IM,JM,7/))
    if (associated(MISRFQ18000))    MISRFQ18000    = reshape(fq_MISR(:,:,16),(/IM,JM,7/))
    ...
    if (self%nmask_vars > 0) then
       ! get orb bundle
       call ESMF_StateGet(IMPORT,'SATORB',BUNDLE,__RC__)
       call MAPL_Get(STATE,ExportSpec=ExportSpec,__RC__)
       do i=1,self%nmask_vars
          vindex = MAPL_VarSpecGetIndex(ExportSpec,self%export_name(i),__RC__)
          call MAPL_VarSpecGet(ExportSpec(vindex),Dims=MAPL_dims,ungridded_dims=Ungridded_dims,__RC__)

          ! get mask from bundle
          call ESMFL_BundleGetPointerToData(BUNDLE,self%mask_name(i),ptr_mask,__RC__)

          ! export maskede variable to new name?
          if (self%newvar(i)) then
             ...
             if (MAPL_dims == MAPL_DimsHorzOnly .and. (.not.associated(ungridded_dims))) then
                call MAPL_GetPointer(EXPORT,ptr2d_new,self%newvar_name(i),__RC__)
                if (associated(ptr2d_new)) then
                   call MAPL_GetPointer(EXPORT,ptr2d,self%export_name(i),__RC__)
                   ptr2d_new = ptr2d
                   where(ptr_mask == MAPL_UNDEF) ptr2d_new = MAPL_UNDEF
                   nullify(ptr2d_new)
                   nullify(ptr2d)
                end if
             elseif (MAPL_dims == MAPL_DimsHorzOnly .and. associated(ungridded_dims)) then
                call MAPL_GetPointer(EXPORT,ptr3d_new,self%newvar_name(i),__RC__)
                if (associated(ptr3d_new)) then
                   call MAPL_GetPointer(EXPORT,ptr3d,self%export_name(i),__RC__)
                   ptr3d_new = ptr3d
                   do j=1,ungridded_dims(1)
                      where(ptr_mask == MAPL_UNDEF) ptr3d_new(:,:,j) = MAPL_UNDEF
                   nullify(ptr3d_new)
                   nullify(ptr3d)
                   end do
                end if
             elseif (MAPL_dims == MAPL_DimsHorzVert) then
                call MAPL_GetPointer(EXPORT,ptr3d_new,self%newvar_name(i),__RC__)
                if (associated(ptr3d_new)) then
                   call MAPL_GetPointer(EXPORT,ptr3d,self%export_name(i),__RC__)
                   ptr3d_new = ptr3d
             ...
             ! determine what dimension pointer we need
             if (MAPL_dims == MAPL_DimsHorzOnly .and. (.not.associated(ungridded_dims))) then
                call MAPL_GetPointer(EXPORT,ptr2d,self%newvar_name(i),__RC__)
                if (associated(ptr2d)) then
                   where(ptr_mask == MAPL_UNDEF) ptr2d = MAPL_UNDEF
                   nullify(ptr2d)
                end if
             elseif (MAPL_dims == MAPL_DimsHorzOnly .and. associated(ungridded_dims)) then
                call MAPL_GetPointer(EXPORT,ptr3d,self%newvar_name(i),__RC__)
                if (associated(ptr3d)) then
                   do j=1,ungridded_dims(1)
                      where(ptr_mask == MAPL_UNDEF) ptr3d(:,:,j) = MAPL_UNDEF
                   end do
                   nullify(ptr3d)
                end if
             elseif (MAPL_dims == MAPL_DimsHorzVert) then
