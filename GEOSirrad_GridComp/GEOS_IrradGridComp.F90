...
module GEOS_IrradGridCompMod
   ...
   ! Whether a 3-D CO2 import is needed; set once in SetServices and
...
contains
   ...
   subroutine SetServices(gc, rc)
      ...
      !DESCRIPTION: Sets the Run entry point and registers the IMPORT, EXPORT,
      ! and internal state variables generated (via ACG) from
      ! Irrad_StateSpecs.rc.
      !EOP
...
#include "Irrad_Internal___.h"
      ...
      ! These fields are genuinely dynamic (names/count come from the
      ...
      stdname = "sensitivity_of_net_downward_longwave_flux_in_air_to_surface_temperature"
      call MAPL_GridCompAddSpec(gc, &
           state_intent=ESMF_STATEINTENT_INTERNAL, &
      ...
      do i = 1, n
         ...
         stdname = "sensitivity_of_net_downward_longwave_flux_in_air_to_surface_temperature_due_to"
         call MAPL_GridCompAddSpec(gc, &
              state_intent=ESMF_STATEINTENT_INTERNAL, &
              short_name='DFDTS_' // trim(nameRATS(i)), &
              standard_name=stdname // trim(nameRATS(i)), &
              units='W m-2 K-1', &
              dims='xyz', &
              vertical_stagger=MAPL_VERTICAL_STAGGER_EDGE, &
   ...
   subroutine Run(gc, import, export, clock, rc)
      ...
      ! Internal-state pointer declarations generated by ACG from Irrad_StateSpecs.rc
#include "Irrad_DeclarePointer___.h"

      ! NOTE: pchakrab
      ! Scratch pointers for the Edge-array bounds remap below. gfortran-15
      ! rejects a *self*-referencing rank remap (X(bounds) => X) with
      ! "Rank remapping target must be rank 1 or simply contiguous". Routing
      ! through a distinct CONTIGUOUS-declared scratch pointer avoids this
      ! TODO: pchakrab - this is ok for now, but we should eventually remove
      ! the bounds remap, since we assume here that the edge arrays are always
      ! contiguous, which is not guaranteed to be the case. The proper fix is
      ! have the code operate on 1-based edge arrays
      ...
      real, pointer, contiguous, dimension(:, :, :, :) :: DFDTS_RAT
      ...
      ! Test to see if irrad.yaml is set up correctly for the Radiation selected
...
#include "Irrad_GetPointer___.h"

      ! MAPL always creates Edge-staggered fields with Fortran bounds
      ! 1:LM+1 (see FieldCreate.F90's make_bounds_from_args -
      ! LU_Bound(1,num_levels) - there is no automatic 0-based remap for
      ! VLOC=E fields). This code assumes the 0:LM layer-interface
      ! convention throughout (e.g. FLX_INT(:,:,0)/(:,:,LM)), so remap
      ! each Edge pointer here, once, right after the ACG fetch above.
      ! These are all mandatory (INTERNAL/no-COND IMPORT) fields, always
      ! associated, so no associated() guard is needed here (contrast
      ! with the EXPORT-side remap in Update_Flx, which does need one).
      p3d => FLX_INT;      FLX_INT(1:IM,1:JM,0:LM)      => p3d
      p3d => FLC_INT;      FLC_INT(1:IM,1:JM,0:LM)      => p3d
      p3d => FLA_INT;      FLA_INT(1:IM,1:JM,0:LM)      => p3d
      p3d => FLXD_INT;     FLXD_INT(1:IM,1:JM,0:LM)     => p3d
      p3d => FLXU_INT;     FLXU_INT(1:IM,1:JM,0:LM)     => p3d
      p3d => FLCD_INT;     FLCD_INT(1:IM,1:JM,0:LM)     => p3d
      p3d => FLCU_INT;     FLCU_INT(1:IM,1:JM,0:LM)     => p3d
      p3d => FLAD_INT;     FLAD_INT(1:IM,1:JM,0:LM)     => p3d
      p3d => FLAU_INT;     FLAU_INT(1:IM,1:JM,0:LM)     => p3d
      p3d => FLXA_INT;     FLXA_INT(1:IM,1:JM,0:LM)     => p3d
      p3d => FLXAD_INT;    FLXAD_INT(1:IM,1:JM,0:LM)    => p3d
      p3d => FLXAU_INT;    FLXAU_INT(1:IM,1:JM,0:LM)    => p3d
      p3d => DFDTS;        DFDTS(1:IM,1:JM,0:LM)        => p3d
      p3d => DFDTSC;       DFDTSC(1:IM,1:JM,0:LM)       => p3d
      p3d => DFDTSNA;      DFDTSNA(1:IM,1:JM,0:LM)      => p3d
      p3d => DFDTSCNA;     DFDTSCNA(1:IM,1:JM,0:LM)     => p3d
      p3d => PLE;          PLE(1:IM,1:JM,0:LM)          => p3d
      p1d => PREF;         PREF(0:LM)                   => p1d

      ! Determine calling sequence
      call MAPL_GridCompGetResource(gc, "CALLED_LAST", called_last, default=1, _RC)
   ...
   contains
      ...
      subroutine LW_Driver(IM, JM, LM, lats, lons, rc)
         ...
         use mo_cloud_sampling, only: sampled_mask_exp_ran, sampled_urand_gen_max_ran
         use mo_optical_props, only: ty_optical_props, ty_optical_props_arry, ty_optical_props_1scl
         use mo_optical_props, only: ty_optical_props_2str, ty_optical_props_nstr
         ...
         use cloud_subcol_gen, only : correlation_length_cloud_fraction, correlation_length_condensate
         ...
         real, dimension(IM, JM, LM, 5), target :: CWC ! in-cloud cloud water mixing ratio
         real, dimension(IM, JM, LM, 5), target :: REFF ! effective radius of cloud particles
         ...
         ! CFC12, HCFC22, FCLD_IN, TS, EMIS, PREF are host-associated from Run,
...
#define TEST_(A) error_msg = A; if (trim(error_msg)/="") then; _ASSERT(.false.,"RRTMGP Error: "//trim(error_msg)); endif
         ...
         if (CO2_FIXED == -2.0) then ! 3D CO2
            if (USE_CHOU) then ! No 3D CO2 if USE_CHOU
               ...
               ! from the state, so we deliberately use plain RC=STATUS here WITHOUT
               ! verifying it - a missing-item failure is silently ignored and
               ! tmp_3d stays nullified, matching the old NotFoundOK=.true. behavior.
               ! Do not change this to use that verifying macro.
               call MAPL_StateGetPointer(export, tmp_3d, 'CO2_FIXED', rc=status)
         ...
         SCHEME: if (USE_CHOU) then
            ...
            if (associated(CO2_3d)) then
               TEST_(gas_concs%set_vmr('co2', real(reshape(CO2_3d, [ncol, LM]), kind=wp))) !<<>> MSL
            end if
            ...
            do K = 1, LM - 1
               ...
               t_lev(:, K + 1) = &
                    (t_lay(:, K) * dp_wp(:, K + 1) + t_lay(:, K + 1) * dp_wp(:, K)) / &
                    (dp_wp(:, K + 1) + dp_wp(:, K))
               dzmid(:, K) = &
                    t_lev(:, K + 1) * real(MAPL_RGAS / MAPL_GRAV, kind=wp) * &
                    (p_lay(:, K + 1) - p_lay(:, K)) / p_lev(:, K + 1)
            end do
            ...
            if (need_cloud_optical_props) then
               ...
               call MAPL_GridCompGetResource(gc, "RRTMGP_ICE_ROUGHNESS_LW", icergh, default=2, _RC)
            ...
            SFCEM_INT = real(reshape( &
                 - flux_up_allsky(:, LM + 1) &
                 + flux_dn_allsky(:, LM + 1) * (1._wp - emis_sfc(1, :)), [IM, JM]))
            ...
            if (LM > 72) then
               call MAPL_GridCompGetResource(gc, &
                    "RRTMGLW_USE_PRECIP_IN_RADIATION", &
                    USE_PRECIP_IN_RADIATION, &
                    default=.true., _RC)
            else
               call MAPL_GridCompGetResource(gc, &
                    "RRTMGLW_USE_PRECIP_IN_RADIATION", &
               ...
               do i = 1, IM
                  ...
                  do K = 1, LM
                     ...
                     if (USE_PRECIP_IN_RADIATION) then
                        ...
                        if (LWT > 0.0) then
                           RELIQ(IJ, K) = ( &
                                REFF(i, j, LV, KLIQUID) * CWC(i, j, LV, KLIQUID) + &
                        ...
                        if (IWT > 0.0) then
                           REICE(IJ, K) = ( &
                                REFF(i, j, LV, KICE) * CWC(i, j, LV, KICE) + &
            ...
            do n = 1, nRATS !<<>> MSL
               ...
               call rrtmg_lw(IM * JM, LM, PARTITION_SIZE, Ts_derivs, &
                    ...
                    UFLXRAT(:, :, n), DFLXRAT(:, :, n), UFLXC, DFLXC, &
                    DUFLX_DT_RAT(:, :, n), DUFLXC_DTS, &
      ...
      subroutine Update_Flx(IM, JM, LM, rc)
         ...
         ! after its own ACG-generated Irrad_GetPointer___.h include).
         ! Guarded by associated() since export pointers can be null if
         ! not requested downstream.
         if (associated(FLX))   then; p3d => FLX;   FLX  (1:IM,1:JM,0:LM) => p3d; end if
         if (associated(FLXA))  then; p3d => FLXA;  FLXA (1:IM,1:JM,0:LM) => p3d; end if
         if (associated(FLC))   then; p3d => FLC;   FLC  (1:IM,1:JM,0:LM) => p3d; end if
         if (associated(FLA))   then; p3d => FLA;   FLA  (1:IM,1:JM,0:LM) => p3d; end if
         if (associated(FLXU))  then; p3d => FLXU;  FLXU (1:IM,1:JM,0:LM) => p3d; end if
         if (associated(FLXAU)) then; p3d => FLXAU; FLXAU(1:IM,1:JM,0:LM) => p3d; end if
         if (associated(FLCU))  then; p3d => FLCU;  FLCU (1:IM,1:JM,0:LM) => p3d; end if
         if (associated(FLAU))  then; p3d => FLAU;  FLAU (1:IM,1:JM,0:LM) => p3d; end if
         if (associated(FLXD))  then; p3d => FLXD;  FLXD (1:IM,1:JM,0:LM) => p3d; end if
         if (associated(FLXAD)) then; p3d => FLXAD; FLXAD(1:IM,1:JM,0:LM) => p3d; end if
         ...
         ! when no coupler requested it as an export, so it is computed into a local
         ! allocatable array and copied into the real export pointer (if any) at the
         ! end of this routine.
         allocate(CLDTT(IM, JM), _STAT)
         call MAPL_StateGetPointer(export, CLDTT_ptr, 'CLDTT', _RC)
         ...
         if (nRATS > 0) then
            ...
            do n = 1, nRATS
               ...
               ! NOTE: nameRATS is the list of active RAT toggles read from config file
               gen_str = 'dOLR_' // trim(nameRATS(n))
               ...
               gen_str = 'NETTRAP_' // trim(nameRATS(n))
               call MAPL_StateGetPointer(export, RAT_2D, trim(gen_str), rc=status) ! Don't verify.
               if (associated(RAT_2D)) then
               ...
               gen_str = 'COLTRAP_' // trim(nameRATS(n))
               ! Uncomment the following line to multiply by area
               ! call MAPL_StateGetPointer(import, AREA, 'AREA', _RC)
               call MAPL_StateGetPointer(export, RAT_3D, trim(gen_str), rc=status) ! Don't verify.
               if (associated(RAT_3D)) then
                  do K = 1, LM
                     RAT_3D(:, :, K) = (FLX_INT(:, :, K)) - (FLX_INT(:, :, K - 1))
                     RAT_3D(:, :, K) = ( &
                          RAT_3D(:, :, K) - &
                          ((FLX_INT_RAT(:, :, K, n)) - (FLX_INT_RAT(:, :, K - 1, n))) &
                          )
                     ! Uncomment the following line to multiply by area
                     ! RAT_3D(:, :, K) = AREA(:,:) * (RAT_3D(:, :, K))
                  end do
                  RAT_3D => null()
                  ! AREA => null() ! Uncomment for AREA
               end if
               gen_str = 'FLX_' // trim(nameRATS(n))
               call MAPL_StateGetPointer(export, RAT_3D, trim(gen_str), rc=status) ! Don't verify.
               if (associated(RAT_3D)) then
                  RAT_3D = FLX_INT(:, :, :) - FLX_INT_RAT(:, :, :, n)
                  RAT_3D => null()
               end if
               gen_str = 'DFDTS_' // trim(nameRATS(n))
               call MAPL_StateGetPointer(export, RAT_3D, trim(gen_str), rc=status) ! Don't verify.
               if (associated(RAT_3D)) then
                  ...
                  if (cond_inhomo) then
                     ...
                     do igpt = 1, ngpt
                        if (cld_mask(isub, ilay, igpt)) then
                           zcw(isub, ilay, igpt) = zcw_lookup(real(urand_cond(igpt, ilay, isub)), sigma_qcw)
                        end if
                     end do
                  end if
               end if
...
#define TEST_(msg) if (msg /= '') then; write(0,*) trim(msg); VERIFY_(STATUS); end if
      ...
      if (need_dirty_optical_props) then
         call compute_lw_aer_optics(colS, colE, TAUA_3d, SSAA_3d, ASYA_3d, aer_props, _RC)
      end if
...
subroutine Irrad_SetServices(gc, rc)
   use ESMF
   use GEOS_IrradGridCompMod, only: mySetServices => SetServices
   type(ESMF_GridComp) :: gc
   integer, intent(out) :: rc
   call mySetServices(gc, rc=rc)
end subroutine Irrad_SetServices
