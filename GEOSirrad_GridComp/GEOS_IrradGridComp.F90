...
#include "Irrad_DeclarePointer___.h"
      ...
      ! "Rank remapping target must be rank 1 or simply contiguous", even
      ! though the ACG-generated declarations above are already CONTIGUOUS
      ! (confirmed: CONTIGUOUS is required but not sufficient when the
      ! pointer is its own data-target). Routing through a distinct
      ! CONTIGUOUS-declared scratch pointer avoids this - verified to
      ! compile and run correctly with gfortran-15 in isolation before
...
#define TEST_(A) error_msg = A; if (trim(error_msg)/="") then; _ASSERT(.false.,"RRTMGP Error: "//trim(error_msg)); endif
         ...
         RADIATIVELY_ACTIVE_AEROSOLS: if (implements_aerosol_optics) then
            ...
            call ESMF_InfoGet(aero_info, key='relative_humidity_for_aerosol_optics', value=AS_FIELD_NAME, _RC)
            ...
            call ESMF_InfoGet(aero_info, key='air_pressure_for_aerosol_optics', value=AS_FIELD_NAME, _RC)
            ...
            IR_BANDS: do band = 1, NB_IRRAD
               ...
               call ESMF_InfoGet(aero_info, key='extinction_in_air_due_to_ambient_aerosol', value=AS_FIELD_NAME, _RC)
               ...
               call ESMF_InfoGet(aero_info, key='single_scattering_albedo_of_ambient_aerosol', value=AS_FIELD_NAME, _RC)
               ...
               call ESMF_InfoGet(aero_info, key='asymmetry_parameter_of_ambient_aerosol', value=AS_FIELD_NAME, _RC)
         ...
         SCHEME: if (USE_CHOU) then
            ...
            call MAPL_GridCompGetResource(gc, "RRTMGP_GAS_LW", k_dist_file, default='rrtmgp-gas-lw-g128.nc', _RC)
            ...
            if (need_cloud_optical_props) then
               ...
               call MAPL_GridCompGetResource(gc, "RRTMGP_CLOUD_OPTICS_LW", cloud_optics_file, &
               ...
               call MAPL_GridCompGetResource(gc, "RRTMGP_CLOUD_OVERLAP_TYPE_LW", cloud_overlap_type, &
