...
contains
   ...
   subroutine SetServices(GC, RC)
      ...
      if (DT == -2.0) then
         ...
         if (n <= 0 .or. ESMF_UtilStringLowerCase(trim(gen_str)) == 'none') then
...
#include "Irrad_Internal___.h"
      ...
      if (STATUS == ESMF_SUCCESS .and. n /= 0) then ! if the label was found...
         ...
         do i = 1, n
            ...
            call MAPL_AddInternalSpec(GC, &
                 ...
                 LONG_NAME='sensitivity_of_net_downward_longwave_flux_in_air_to_surface_temperature_due_to' // trim(&
                 nameRATS(i)),&
                 UNITS='W m-2 K-1', &
                 DIMS=MAPL_DimsHorzVert, &
                 add2export=.true., &
   ...
   subroutine Run(GC, IMPORT, EXPORT, CLOCK, RC)
      ...
      character(len=6), dimension(8) :: RATNAMES = (/'O3    ', 'N2O   ', 'CFC11 ', 'CFC12 ', 'CH4   ', 'HCFC22', &
           'H2O   ', 'CO2   '/)
...
#define TEST_(A) error_msg = A; if (trim(error_msg)/="") then; _ASSERT(.false.,"RRTMGP Error: "//trim(error_msg)); endif
         ...
         call compute_provider_aerosol_optics(AERO, &
              RH, PLE, IM, JM, LM, NB_IRRAD, OFFSET, &
              NA, TAUA, SSAA, ASYA, &
         ...
         SCHEME: if (USE_CHOU) then
            ...
            ! from RRTMG:
            ! wavenum1(:) = (/ 10., 350., 500., 630., 700., 820.,  980., 1080., 1180., 1390., 1480., 1800., 2080.,
            ! 2250., 2380., 2600./)
            ! wavenum2(:) = (/350., 500., 630., 700., 820., 980., 1080., 1180., 1390., 1480., 1800., 2080., 2250.,
            ! 2380., 2600., 3250./)
            ! from RRTMGP:
            ! write(*,*) 'band_lims_wvn(2,nbnd):', k_dist%get_band_lims_wavenumber()  ! with output reordered
            !                  10., 250., 500., 630., 700., 820.,  980., 1080., 1180., 1390., 1480., 1800., 2080.,
            ! 2250., 2390., 2680.
            !                 250., 500., 630., 700., 820., 980., 1080., 1180., 1390., 1480., 1800., 2080., 2250.,
            ! 2390., 2680., 3250.
            ...
            if (first) then
               ...
               ! No error thrown. Just go around this if nothing learnable from config.
               if (STATUS == ESMF_SUCCESS) then ! if the label was found...
            ...
            if (LM > 72) then
               call MAPL_GetResource(MAPL, USE_PRECIP_IN_RADIATION, 'RRTMGLW_USE_PRECIP_IN_RADIATION:', default=.true.&
                    &, _RC)
            else
               call MAPL_GetResource(MAPL, USE_PRECIP_IN_RADIATION, 'RRTMGLW_USE_PRECIP_IN_RADIATION:', default=.false.&
                    , _RC)
            end if
...
#undef TEST_
   ...
   ! compute_provider_aerosol_optics: query the AERO provider state for
   !   per-band extinction/single-scattering-albedo/asymmetry-parameter
   !   aerosol optics and load them into TAUA/SSAA/ASYA. Extracted from
   !   LW_Driver's RADIATIVELY_ACTIVE_AEROSOLS block, mirroring the MAPL3
   !   port's compute_provider_aerosol_optics extraction of the same block
   !   (MAPL2's ESMF_AttributeGet/ESMF_MethodExecute API here in place of
   !   MAPL3's ESMF_Info-based one).
   subroutine compute_provider_aerosol_optics(AERO, & ! input/output
        ...
        implements_aerosol_optics, RC)                ! output
      ...
      logical, intent(out) :: implements_aerosol_optics
      integer, optional, intent(out) :: RC
      ...
      IAm = "compute_provider_aerosol_optics"
      ...
      RADIATIVELY_ACTIVE_AEROSOLS: if (implements_aerosol_optics) then
         ...
         IR_BANDS: do band = 1, NB_IRRAD
            ...
            call ESMF_AttributeGet(AERO, NAME='extinction_in_air_due_to_ambient_aerosol', value=AS_FIELD_NAME, RC=&
                 STATUS)
            VERIFY_(STATUS)
            ...
            call ESMF_AttributeGet(AERO, NAME='single_scattering_albedo_of_ambient_aerosol', value=AS_FIELD_NAME, RC&
                 =STATUS)
            VERIFY_(STATUS)
            ...
            call ESMF_AttributeGet(AERO, NAME='asymmetry_parameter_of_ambient_aerosol', value=AS_FIELD_NAME, RC=&
                 STATUS)
            VERIFY_(STATUS)
...
#define TEST_(msg) if (msg /= '') then; write(0,*) trim(msg); VERIFY_(STATUS); end if
      ...
      IAm = "compute_lw_cloud_optics_mcica"
...
#define TEST_(msg) if (msg /= '') then; write(0,*) trim(msg); VERIFY_(STATUS); end if
      ...
      real(kind=wp), dimension(:, :), intent(inout), target, optional :: flux_up_clrnoa, flux_dn_clrnoa, &
           dfupdts_clrnoa
      real(kind=wp), dimension(:, :), intent(inout), target, optional :: flux_up_allnoa, flux_dn_allnoa, &
           dfupdts_allnoa
      real(kind=wp), dimension(:, :), intent(inout), target, optional :: flux_up_clrsky, flux_dn_clrsky, &
           dfupdts_clrsky
      real(kind=wp), dimension(:, :), intent(inout), target, optional :: flux_up_allsky, flux_dn_allsky, &
           dfupdts_allsky
